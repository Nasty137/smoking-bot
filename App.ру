...
# --- Логика бота (ваш код из smoking_bot.py) ---
# Ниже я скопировал ваш код. Он полностью идентичен, только в конце
# вместо asyncio.run(main()) мы запускаем бота в отдельном потоке
...
# --- Запуск бота в отдельном потоке + Flask для Render ---
def run_bot():
    asyncio.run(main())
